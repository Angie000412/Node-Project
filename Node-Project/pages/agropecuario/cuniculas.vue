...
<template>
  <v-data-table :headers="headers" :items="cuniculas" :search="search" sort-by="calories" class="elevation-18 mx-auto">
    <template #top>
      <v-toolbar flat>
        <v-toolbar-title>
          <v-icon>mdi-rabbit</v-icon>Cuniculas
        </v-toolbar-title>
        <v-divider class="mx-4" inset vertical></v-divider>
        <v-text-field v-model="search" class="justify-center" append-icon="mdi-magnify" label="Search" single-line
          hide-details></v-text-field>
        <v-divider class="mx-4" inset vertical></v-divider>
        <v-dialog v-model="dialog" max-width="500px">
          <template #activator="{ on, attrs }">
            <v-btn color="blue lighten-1" class="mb-2 p-2" v-bind="attrs" v-on="on">
              Añadir entidad
            </v-btn>
            &nbsp;&nbsp;&nbsp;
            ...
            <v-menu bottom offset-y>
              <template v-slot:activator="{ on, attrs }">
                <v-btn class="mb-2 p-2" v-bind="attrs" v-on="on" color="blue lighten-1">
                  Ver Potencial <v-icon append>mdi-chevron-down</v-icon>
                </v-btn>
              </template>
              <v-list>
                <v-list-item v-for="(item, i) in potenciales" :key="i" :to="item.to" @click="() => { }">
          ...
          <v-card>
            ...
            <v-card-text>
              <v-form v-model="form" ref="form">
                ...
                <v-row>
                  ...
                  <v-col>
                    <v-text-field :rules="[rules.required, rules.length(1), rules.nombre_entidad]"
                      v-model="editedItem.nombre_entidad" label="Nombre Entidad"></v-text-field>
                  </v-col>
                </v-row>
                <v-row>
                  <v-col>
                    <v-select :items="provincias" @change="llenarMunicipios()" :rules="[v => !!v || 'Campo requerido']" v-model="Completar.provincia"
                      item-text="provincia" label="seleccione una provincia" clearable></v-select>
                  </v-col>
                  <v-col>
                    <v-select :items="municipios" @change="verMuni()" item-text="municipio"
                     :rules="[v => !!v || 'Campo requerido']" v-model="Completar.municipio" label="seleccione un municipio" clearable></v-select>
                  </v-col>
                </v-row>
                <v-row>

                  <v-col>
                    <v-text-field type="number" :rules="[rules.required, rules.cantidades]"
                      v-model="editedItem.numero_reproductoras" label="Numero Reproductoras"></v-text-field>
                  </v-col>
                  <v-col>
                    <v-text-field type="number" :rules="[rules.required, rules.cantidades]"
                      v-model="editedItem.numero_sementales" label="Numero de Sementales"></v-text-field>
                  </v-col>
                  <v-col>
                    <v-text-field type="number" :rules="[rules.required, rules.cantidades]"
                      v-model="editedItem.numero_crias" label="Numero de Crias"></v-text-field>
                  </v-col>
                </v-row>
                ...
                <v-row>
                  <v-col>
                    <v-text-field type="number" :rules="[rules.required, rules.latitud]" v-model="editedItem.latitud"
                      label="latitud"></v-text-field>
                  </v-col>
                  <v-col>
                    <v-text-field type="number" :rules="[rules.longitud]" v-model="editedItem.longitud"
            ...
            <v-card-actions>
              ...
              <v-btn color="blue lighten-1" @click="agregar" :disabled="!form" :loading="isLoading"> Save </v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            <v-card-title class="text-h5" color="blue lighten-1">Seguro que desea eliminar la entidad?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="blue lighten-1" @click="closeDelete">Cancel</v-btn>
              <v-btn color="blue lighten-1" @click="deleteItemConfirm">OK</v-btn>
              <v-spacer></v-spacer>
            </v-card-actions>
          </v-card>
        </v-dialog>
    ...
    <template #item.actions="{ item }">
      ...
      <v-btn color="error" x-small @click="deleteItem(item)">
...
export default {
  ...
  data() {
    return {
      ...
      potenciales: [
        {
          title: 'Disponible',
          to: '/Agropecuario/potencial/potencial_cuniculas'
        },
        {
          title: 'Implementable',
          to: '/Agropecuario/uso_final/uso_final_cuniculas'
        },
  ...
  computed: {
    ...
    PotencialMin() {//no lo calcula hasta que no estén los tres escritos en el editedItem
      var pdmin = (7.63 * parseFloat(this.editedItem.numero_reproductoras) * 1 / 1000 * 33) +
        (11.18 * parseFloat(this.editedItem.numero_sementales) * 1 / 1000 * 33);
    ...
    PotencialMax() {//no lo calcula hasta que no estén los tres escritos en el editedItem
      var pdmax = (7.63 * parseFloat(this.editedItem.numero_reproductoras) * 1 / 1000 * 40) +
        (11.18 * parseFloat(this.editedItem.numero_sementales) * 1 / 1000 * 40)
  ...
  methods: {
    async initialize() {
      ...
      await axios.get('http://localhost:1337/api/cuniculas/').then((response) => {
    ...
    editItem(item) {
      this.editedIndex = this.cuniculas.indexOf(item)
    ...
    deleteItemConfirm() {
      /* this.cuniculas.splice(this.editedIndex, 1)
      this.closeDelete() */
      var id = this.editedItem.id
      axios.delete('http://localhost:1337/api/cuniculas/' + id + '/')
        .then(response => {
    ...
    agregar() {
      ...
      if (this.editedIndex > -1) {
        ...
        var config = {
          method: 'put',
          url: 'http://localhost:1337/api/cuniculas/' + id + '/',
          headers: {
        ...
        var config = {
          method: 'post',
          url: 'http://localhost:1337/api/cuniculas',
          headers: {
            'Content-Type': 'application/json'
          },
          data: data
