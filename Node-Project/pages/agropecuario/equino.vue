...
<template>
  ...
  >
    <template #top>
      <v-toolbar flat>
        <v-toolbar-title><v-icon>mdi-horse</v-icon>Equinos</v-toolbar-title>
...
export default {
  ...
  data(){
    ...
    potenciales: [
            {
              title: 'Disponible',
              to: '/Agropecuario/potencial/potencial_equino'
            },
            {
              title: 'Implementable',
              to: '/Agropecuario/uso_final/uso_final_equino'
            },
  ...
  computed: {
    ...
    PotencialMin(){//no lo calcula hasta que no estén los tres escritos en el editedItem
        const pesoPromedioPorCaballo = 450;
        const excretaPromedioMin = 10;
        const estiercolPorCabeza= excretaPromedioMin *parseFloat (this.editedItem.cant_cabezas) ;
       
       
        const pdmin= estiercolPorCabeza * 0.04*0.05*10
      ...
      PotencialMax(){
        const excretaPromedioMin = 15;
        const estiercolPorCabeza= excretaPromedioMin *parseFloat (this.editedItem.cant_cabezas) ;
             var pdmax=  estiercolPorCabeza * 0.04*0.05 *10 //00 potencial biogas entidades pecuarias
  ...
  methods: {
    ...
    async initialize() {
      ...
      await axios.get('http://localhost:1337/api/equinos/').then((response) => {
          response.data.data.forEach((item) => {
            this.equinos.push({
...
console.log(this.Completar.municipio);
    ...
    editItem(item) {
      this.editedIndex = this.equinos.indexOf(item)
    ...
    deleteItemConfirm() {
      /* this.equinos.splice(this.editedIndex, 1)
      this.closeDelete() */
      var id =this.editedItem.id
                axios.delete('http://localhost:1337/api/equinos/'+id+'/')
                .then(response=>{
...
var config = {
  method: 'put',
  url: 'http://localhost:1337/api/equinos/'+id+'/',
  headers: { 
...
var config = {
  method: 'post',
  url: 'http://localhost:1337/api/equinos',
  headers: { 
    'Content-Type': 'application/json'
  },
  data : data
