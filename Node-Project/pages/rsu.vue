...
<template>
  ...
  >
    <template #top>
      <v-toolbar flat>
        <v-toolbar-title><v-icon>mdi-home-variant</v-icon>Residuos Urbanos</v-toolbar-title>
        ...
        label="seleccione un municipio" clearable></v-select>
              ...
              <v-row> 
                ...
                <v-col>
                  ...
                  type="number"
                  :rules="[rules.required,rules.cantidades]"
                  v-model="editedItem.cant_residuos"
...
export default {
  ...
  data(){
    ...
    headers: [
     /*  {
      ...
      { text: 'Catidad de Residuos', value: 'cant_residuos' },
    ...
    potenciales: [
            {
              title: 'Disponible',
              to: '/industrial/potencial/potencial_rsu'
            },
            {
              title: 'Implementable',
              to: '/industrial/uso_final/uso_final_rsu'
            },
  ...
  computed: {
    ...
    PotencialMin(){//no lo calcula hasta que no estén los tres escritos en el editedItem
            var pdmin=(7.63* parseFloat(this.editedItem.habitantes) *1 / 1000 * 33) +
             (11.18* parseFloat (this.editedItem.cant_residuos) *1 / 1000 * 33) ;
    ...
    PotencialMax(){//no lo calcula hasta que no estén los tres escritos en el editedItem
            var pdmax=(7.63* parseFloat(this.editedItem.habitantes) *1 / 1000 * 40) +
             (11.18* parseFloat (this.editedItem.cant_residuos) *1 / 1000 * 40) 
        ...
        .then((response) => {
          response.data.data.forEach((item) => {
            this.rsus.push({
              ...
              habitantes: item.attributes.habitantes,
              cant_residuos: item.attributes.cant_residuos,
...
,
    ...
    deleteItemConfirm() {
      /* this.rsus.splice(this.editedIndex, 1)
      this.closeDelete() */
      var id =this.editedItem.id
                axios.delete('http://localhost:1337/api/rsus/'+id+'/')
                .then(response=>{
  ...
  "data": {  
    ...
    "habitantes":this.editedItem.habitantes,
    "cant_residuos": this.editedItem.cant_residuos,
...
var config = {
  method: 'put',
  url: 'http://localhost:1337/api/rsus/'+id+'/',
  headers: { 
...
var config = {
  method: 'post',
  url: 'http://localhost:1337/api/rsus',
  headers: { 
    'Content-Type': 'application/json'
  },
  data : data
