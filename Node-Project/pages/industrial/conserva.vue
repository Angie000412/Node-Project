...
<template>
  ...
  >
    <template #top>
      <v-toolbar flat>
        <v-toolbar-title><v-icon>mdi-fridge</v-icon>Conservas</v-toolbar-title>
      ...
      <v-select :items="municipios" @change="verMuni()" item-text="municipio" :rules="[v => !!v || 'Campo requerido']" v-model="Completar.municipio"
        label="seleccione un municipio" clearable></v-select>
              ...
              <v-row> 
                ...
                <v-col>
                  <v-select
                  :items="procesamientos"
                  item-text="procesamiento"
...
export default {
  ...
  data(){
    ...
    headers: [
     /*  {
      ...
      { text: 'Procesamiento', value: 'procesamiento' },
    ...
    potenciales: [
            {
              title: 'Disponible',
              to: '/industrial/potencial/potencial_conservas'
            },
            {
              title: 'Implementable',
              to: '/industrial/uso_final/uso_final_conservas'
            },
  ...
  computed: {
    ...
    PotencialMin(){//no lo calcula hasta que no estén los tres escritos en el editedItem
           if(this.ComProc==='Fruta'){
            var pdmin=parseFloat(this.editedItem.cantidad_residuo)* 95.3 * 29.53 
             } else if (this.ComProc==='Fruta y Vegetal'){
              var pdmin=parseFloat(this.editedItem.cantidad_residuo)* 95.7 * 29.53 
             } 
    ...
    PotencialMax(){//no lo calcula hasta que no estén los tres escritos en el editedItem
      if(this.ComProc==='Fruta'){
        var pdmax=parseFloat(this.editedItem.cantidad_residuo)* 298 * 41.04
             } else if (this.ComProc==='Fruta y Vegetal'){
              var pdmax=parseFloat(this.editedItem.cantidad_residuo)* 469 * 41.04
             } 
  ...
  methods: {
    ...
    await axios
      ...
      await axios
        ...
        .then((response) => {
          response.data.data.forEach((item) => {
            this.conservas.push({
              ...
              procesamiento: item.attributes.procesamiento,
        ...
        await axios
        .get('http://localhost:1337/api/procesamientos/')
        .then((response) => {
          response.data.data.forEach((item) => {
            this.procesamientos.push({
...
,
    ...
    editItem(item) {
      this.editedIndex = this.conservas.indexOf(item)
    ...
    deleteItemConfirm() {
      /* this.conservas.splice(this.editedIndex, 1)
      this.closeDelete() */
      var id =this.editedItem.id
                axios.delete('http://localhost:1337/api/conservas/'+id+'/')
                .then(response=>{
...
var config = {
  method: 'put',
  url: 'http://localhost:1337/api/conservas/'+id+'/',
  headers: { 
...
var config = {
  method: 'post',
  url: 'http://localhost:1337/api/conservas',
  headers: { 
    'Content-Type': 'application/json'
  },
  data : data
