...
<template>
  ...
  >
    <template #top>
      <v-toolbar flat>
        <v-toolbar-title><v-icon>mdi-spoon-sugar</v-icon>Azucareras</v-toolbar-title>
...
export default {
  ...
  data(){
    ...
    headers: [
     /*  {
      ...
      { text: 'Vinaza', value: 'vinaza' },
      { text: 'Cachaza', value: 'cachaza' },
    ...
    potenciales: [
            {
              title: 'Disponible',
              to: '/industrial/potencial/potencial_azucareras'
            },
            {
              title: 'Implementable',
              to: '/industrial/uso_final/uso_final_azucareras'
            },
  ...
  computed: {
    ...
    PotencialMin(){//no lo calcula hasta que no estén los tres escritos en el editedItem
            var pdmin=(7.63* parseFloat(this.editedItem.vinaza) *1 / 1000 * 33) +
             (11.18* parseFloat (this.editedItem.cachaza) *1 / 1000 * 33) ;
    ...
    PotencialMax(){//no lo calcula hasta que no estén los tres escritos en el editedItem
            var pdmax=(7.63* parseFloat(this.editedItem.vinaza) *1 / 1000 * 40) +
             (11.18* parseFloat (this.editedItem.cachaza) *1 / 1000 * 40) 
  ...
  methods: {
    ...
    await axios
      ...
      await axios
        ...
        .then((response) => {
          response.data.data.forEach((item) => {
            this.azucareras.push({

              id: item.id,
              nombre_entidad: item.attributes.nombre_entidad,
              vinaza: item.attributes.vinaza,
              cachaza: item.attributes.cachaza,
...
,
    ...
    editItem(item) {
      this.editedIndex = this.azucareras.indexOf(item)
    ...
    deleteItemConfirm() {
      /* this.azucareras.splice(this.editedIndex, 1)
      this.closeDelete() */
      var id =this.editedItem.id
                axios.delete('http://localhost:1337/api/azucareras/'+id+'/')
                .then(response=>{
  ...
  "data": {  
    ...
    "vinaza":this.editedItem.vinaza,
    "cachaza": this.editedItem.cachaza,
...
var config = {
  method: 'put',
  url: 'http://localhost:1337/api/azucareras/'+id+'/',
  headers: { 
...
var config = {
  method: 'post',
  url: 'http://localhost:1337/api/azucareras',
  headers: { 
    'Content-Type': 'application/json'
  },
  data : data
