...
<template>
    ...
    >
      <template #top>
        <v-toolbar flat>
          <v-toolbar-title><v-icon>mdi-glass-wine</v-icon>Bebidas-Licores</v-toolbar-title>
          ...
          label="seleccione un municipio" clearable></v-select>
                ...
                <v-row> 
                  <v-col >
                    ...
                    v-model="editedItem.agua_residual"
                    label="Agua Residual"
                    ></v-text-field>
                  </v-col>
                  <v-col>
                    ...
                    type="number"
                    :rules="[rules.required,rules.cantidades]"
                    v-model="editedItem.cantidad_producto"
  ...
  export default {
    ...
    data(){
      ...
      headers: [
       /*  {
        ...
        { text: 'Agua Residual', value: 'agua_residual' },
        { text: 'Cantidad Producto', value: 'cantidad_producto' },
      ...
      potenciales: [
              {
                title: 'Disponible',
                to: '/industrial/potencial/potencial_bebidaslicores'
              },
              {
                title: 'Implementable',
                to: '/industrial/uso_final/uso_final_bebidaslicores'
              },
    ...
    computed: {
      ...
      PotencialMin(){//no lo calcula hasta que no estén los tres escritos en el editedItem
              var pdmin=(7.63* parseFloat(this.editedItem.agua_residual) *1 / 1000 * 33) +
               (11.18* parseFloat (this.editedItem.cantidad_producto) *1 / 1000 * 33) ;
      ...
      PotencialMax(){//no lo calcula hasta que no estén los tres escritos en el editedItem
              var pdmax=(7.63* parseFloat(this.editedItem.agua_residual) *1 / 1000 * 40) +
               (11.18* parseFloat (this.editedItem.cantidad_producto) *1 / 1000 * 40) 
    ...
    methods: {
      ...
      await axios
        ...
        await axios
          ...
          .then((response) => {
            response.data.data.forEach((item) => {
              this.bebidaslicores.push({
  
                id: item.id,
                nombre_entidad: item.attributes.nombre_entidad,
                agua_residual: item.attributes.agua_residual,
                cantidad_producto: item.attributes.cantidad_producto,
                tipo_producto: item.attributes.tipo_producto,
                provincia: item.attributes.provincia,
...
,
      ...
      editItem(item) {
        this.editedIndex = this.bebidaslicores.indexOf(item)
      ...
      deleteItemConfirm() {
        /* this.bebidaslicores.splice(this.editedIndex, 1)
        this.closeDelete() */
        var id =this.editedItem.id
                  axios.delete('http://localhost:1337/api/bebidaslicores/'+id+'/')
                  .then(response=>{
    ...
    "data": {  
      ...
      "agua_residual":this.editedItem.agua_residual,
      "cantidad_producto": this.editedItem.cantidad_producto,
      "tipo_producto": this.editedItem.tipo_producto,
  ...
  var config = {
    method: 'put',
    url: 'http://localhost:1337/api/bebidaslicores/'+id+'/',
    headers: { 
  ...
  var config = {
    method: 'post',
    url: 'http://localhost:1337/api/bebidaslicores',
    headers: { 
      'Content-Type': 'application/json'
    },
    data : data
