...
<template>
    ...
    >
      <template #top>
        <v-toolbar flat>
          <v-toolbar-title><v-icon>mdi-rice</v-icon>Arroceras</v-toolbar-title>
          ...
          label="seleccione un municipio" clearable></v-select>
                ...
                <v-row> 
                  <v-col >
                    ...
                    v-model="editedItem.area_cultivo"
                    label="Area dedicada al Cultivo"
                    ></v-text-field>
                  </v-col>
                  <v-col>
                    <v-text-field
                    type="number"
                    :rules="[rules.required,rules.cantidades]"
                    v-model="editedItem.residuo_cosecha"
                    label="Residuo de la cosecha"
                    ></v-text-field>
                  </v-col>
                 </v-row>
  ...
  export default {
    ...
    data(){
      ...
      headers: [
       /*  {
        ...
        { text: 'Área Cultivo', value: 'area_cultivo' },
        { text: 'Residuo Cosecha', value: 'residuo_cosecha' },
      ...
      potenciales: [
              {
                title: 'Disponible',
                to: '/industrial/potencial/potencial_arroceras'
              },
              {
                title: 'Implementable',
                to: '/industrial/uso_final/uso_final_arroceras'
              },
    ...
    computed: {
      ...
      PotencialMin(){//no lo calcula hasta que no estén los tres escritos en el editedItem
              var pdmin=(7.63* parseFloat(this.editedItem.area_cultivo) *1 / 1000 * 33) +
               (11.18* parseFloat (this.editedItem.residuo_cosecha) *1 / 1000 * 33) ;
      ...
      PotencialMax(){//no lo calcula hasta que no estén los tres escritos en el editedItem
              var pdmax=(7.63* parseFloat(this.editedItem.area_cultivo) *1 / 1000 * 40) +
               (11.18* parseFloat (this.editedItem.residuo_cosecha) *1 / 1000 * 40) 
          ...
          .then((response) => {
            response.data.data.forEach((item) => {
              this.arroceras.push({
                ...
                area_cultivo: item.attributes.area_cultivo,
                residuo_cosecha: item.attributes.residuo_cosecha,
...
,
      ...
      editItem(item) {
        this.editedIndex = this.arroceras.indexOf(item)
      ...
      deleteItemConfirm() {
        /* this.arroceras.splice(this.editedIndex, 1)
        this.closeDelete() */
        var id =this.editedItem.id
                  axios.delete('http://localhost:1337/api/arroceras/'+id+'/')
                  .then(response=>{
    ...
    "data": {  
      ...
      "area_cultivo":this.editedItem.area_cultivo,
      "residuo_cosecha": this.editedItem.residuo_cosecha,
  ...
  var config = {
    method: 'put',
    url: 'http://localhost:1337/api/arroceras/'+id+'/',
    headers: { 
  ...
  var config = {
    method: 'post',
    url: 'http://localhost:1337/api/arroceras',
    headers: { 
      'Content-Type': 'application/json'
    },
    data : data
