...
<template>
  ...
  >
    <template #top>
      <v-toolbar flat>
        <v-toolbar-title><v-icon>mdi-sprout-outline</v-icon>Cultivos Varios</v-toolbar-title>
              ...
              <v-form  v-model="form" ref="form">
                  ...
                  <v-col >
                    ...
                    :rules="[rules.required,rules.length(1)]"
                      v-model="editedItem.nombre_cultivo"
        ...
        label="seleccione un municipio" clearable></v-select>
              ...
              <v-row> 
                <v-col >
                  ...
                  v-model="editedItem.hectareas_sembrado"
                  label="Hectareas Sembrado"
                  ></v-text-field>
                </v-col>
                <v-col >
                  <v-text-field
                ...
                <v-row>
                  ...
                  type="number"
                  :rules="[rules.required,rules.cantidades]"
                  v-model="editedItem.produccion_campana"
                  label="Produccion campaña"
                  ></v-text-field>
                </v-col>
                  <v-col >
                    <v-text-field
                    :rules="[rules.required,rules.length(1),rules.epoca]"
                      v-model="editedItem.epoca_campana"
                      label="Epoca campaña"
                    ></v-text-field>
                  </v-col>
                </v-row>
...
export default {
  ...
  data(){
   ...
   validarSelect: v=> !! v  ||  'Campo requerido',
        nombre_entidad: v=> !! v.trim() || 'Campo requerido',
        epoca: v=> !! v.trim() || 'Campo requerido',
        cult: v=> !! v.trim() || 'Campo requerido',
    ...
    headers: [
     /*  {
      ...
      { text: 'Hectareas sembrado', value: 'hectareas_sembrado' },
      { text: 'Tiempo Campaña', value: 'tiempo_campana' },
      { text: 'Producción campaña', value: 'produccion_campana' },
      { text: 'Epoca campaña', value: 'epoca_campana' },
      { text: 'Nombre Cultivo', value: 'nombre_cultivo' },
    ...
    potenciales: [
            {
              title: 'Disponible',
              to: '/industrial/potencial/potencial_cultvar'
            },
            {
              title: 'Implementable',
              to: '/industrial/uso_final/uso_final_cultvar'
            },
  ...
  computed: {
    ...
    PotencialMin(){//no lo calcula hasta que no estén los tres escritos en el editedItem
            var pdmin=(7.63* parseFloat(this.editedItem.hectareas_sembrado) *1 / 1000 * 33) +
             (11.18* parseFloat (this.editedItem.produccion_campana) *1 / 1000 * 33)+
             (4.75* parseFloat (this.editedItem.tiempo_campana) *1 / 1000 * 33) ;
    ...
    PotencialMax(){//no lo calcula hasta que no estén los tres escritos en el editedItem
            var pdmax=(7.63* parseFloat(this.editedItem.hectareas_sembrado) *1 / 1000 * 40) +
             (11.18* parseFloat (this.editedItem.produccion_campana) *1 / 1000 * 40) +
             (4.75* parseFloat (this.editedItem.tiempo_campana) *1 / 1000 * 40) 
  ...
  methods: {
    ...
    await axios
      ...
      await axios
        ...
        .then((response) => {
          response.data.data.forEach((item) => {
            this.cultivosvarios.push({

              id: item.id,
              nombre_entidad: item.attributes.nombre_entidad,

              hectareas_sembrado: item.attributes.hectareas_sembrado,
              produccion_campana: item.attributes.produccion_campana,
              tiempo_campana: item.attributes.tiempo_campana,
              epoca_campana: item.attributes.epoca_campana,
              nombre_cultivo: item.attributes.nombre_cultivo,
...
,
    ...
    editItem(item) {
      this.editedIndex = this.cultivosvarios.indexOf(item)
    ...
    deleteItemConfirm() {
      /* this.cultivosvarios.splice(this.editedIndex, 1)
      this.closeDelete() */
      var id =this.editedItem.id
                axios.delete('http://localhost:1337/api/cultivosvarios/'+id+'/')
                .then(response=>{
  ...
  "data": {  
    ...
    "hectareas_sembrado":this.editedItem.hectareas_sembrado,
    "produccion_campana": this.editedItem.produccion_campana,
    "tiempo_campana": this.editedItem.tiempo_campana,
    "epoca_campana": this.editedItem.epoca_campana,
    "nombre_cultivo": this.editedItem.nombre_cultivo,
...
var config = {
  method: 'put',
  url: 'http://localhost:1337/api/cultivosvarios/'+id+'/',
  headers: { 
...
var config = {
  method: 'post',
  url: 'http://localhost:1337/api/cultivosvarios',
  headers: { 
    'Content-Type': 'application/json'
  },
  data : data
